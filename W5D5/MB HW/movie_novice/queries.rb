...
def below_average_years
  ...
  Movie.select("movies.yr, COUNT(*) as bad_movies").where('score < ?',5).group('yr').order('COUNT(*) desc')
...
def alphabetized_actors
  ...
  Actor.all.select(:id, :name).order('name ASC').limit(10)
...
def pulp_fiction_actors
  ...
  Movie.joins(:actors).where("movies.title like ?", "Pulp Fiction").select("actors.id, actors.name")
...
def uma_movies
  ...
  Movie.joins(:actors).where("actors.name like ?","Uma Thurman").select(:id, :title, :yr).order("movies.yr ASC")
